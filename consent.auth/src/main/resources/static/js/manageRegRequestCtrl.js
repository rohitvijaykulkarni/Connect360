app.controller('manageRegRequestCtrl', ['$scope', '$rootScope','$uibModal', '$http',  '$location',function($scope, $rootScope,$uibModal, $http,$location) {
	...
	//View Status History
	 ...
	 /* $scope.sendStatusHistory = function(selReqID){
		  ...
		  var sendStatHistorydata = {
				  "his_reqreqtid": null,
			        "reqReqtID": selReqID
		}
		
		appReqdata = JSON.stringify(sendStatHistorydata);

    	console.log("viewReq JSON object --- "+sendStatHistorydata);
    	...
    	$http.post("/TransactionController/getRefStatusHistByID/", sendStatHistorydata, config)
		   .then(
		       function(response){
		         ...
		         if(response.data.message){
		        	 ...
		        	 $scope.sucessMsg='';
		        	 $rootScope.sendStatusHistoryData =  response.data;
		        	 console.log($rootScope.sendStatusHistoryData);
